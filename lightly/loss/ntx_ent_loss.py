...
class NTXentLoss(nn.Module):
    ...
    def forward(self, temperature, out0: Tensor, out1: Tensor) -> Tensor:
        ...
        if negatives is not None:
            ...
            logits_00 = (
                torch.einsum("nc,mc->nm", out0, out0_large) / temperature
            )  # self.temperature
            logits_01 = (
                torch.einsum("nc,mc->nm", out0, out1_large) / temperature
            )  # self.temperature
            logits_10 = (
                torch.einsum("nc,mc->nm", out1, out0_large) / temperature
            )  # self.temperature
            logits_11 = (
                torch.einsum("nc,mc->nm", out1, out1_large) / temperature
            )  # self.temperature
...
class InfoNCELoss(nn.Module):
    ...
    Examples:
        ...
        >>>
        >>> # calculate loss
        >>> loss = loss_fn(cat(out0, out1),keys)
    ...
    def forward(self, out: Tensor, keys: Tensor, temperature) -> Tensor:
        """Forward pass through Contrastive Cross-Entropy Loss.
        ...
        Args:
            out:
                Output projections of transformed images.
                Shape: (K*N*batch_size, embedding_size)
            keys:
                keys of transformed images, same key indicates positive.
                Shape: (K*N*batch_size, embedding_size)
        ...
        if self.gather_distributed and dist.world_size() > 1:
            print("Gathering hidden representations from other processes")
            # Gather hidden representations from other processes
            out_large = torch.cat(dist.gather(out), 0)
            keys_large = torch.cat(dist.gather(keys), 0)
            diag_mask = dist.eye_rank(batch_size, device=out.device)
        else:
            # Single process
            out_large = out
            keys_large = keys
            diag_mask = torch.eye(batch_size, device=out.device, dtype=torch.bool)
        sim = torch.exp(
            torch.einsum("if, jf -> ij", out_large, out_large)
            / temperature  # self.temperature
        )
        pos_mask = keys_large.unsqueeze(0).t() == keys_large.unsqueeze(0)
        pos_mask[diag_mask] = 0
        neg_mask = keys_large.unsqueeze(0).t() != keys_large.unsqueeze(0)
