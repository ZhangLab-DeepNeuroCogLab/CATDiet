...
class BaseClassifier(LightningModule, ABC):
    ...
    def training_step(self, batch: Tuple[Tensor, ...], batch_idx: int) -> Tensor:
        ...
        log_dict = {f"train_offline_cls_top{k}": acc for k, acc in topk.items()}
        self.log(
            "train_offline_cls_loss",
            loss,
            prog_bar=True,
            sync_dist=True,
    ...
    def validation_step(self, batch: Tuple[Tensor, ...], batch_idx: int) -> Tensor:
        loss, topk = self.shared_step(batch=batch, batch_idx=batch_idx)
        batch_size = len(batch[1])
        log_dict = {f"val_offline_cls_top{k}": acc for k, acc in topk.items()}
        self.log(
            "val_offline_cls_loss",
            loss,
            prog_bar=True,
            sync_dist=True,
            batch_size=batch_size,
    ...
    def test_step(self, batch: Tuple[Tensor, ...], batch_idx: int) -> Tensor:
        loss, topk = self.shared_step(batch=batch, batch_idx=batch_idx)
        batch_size = len(batch[1])
        log_dict = {f"test_offline_cls_top{k}": acc for k, acc in topk.items()}
        self.log(
            "test_offline_cls_loss",
            loss,
            prog_bar=True,
            sync_dist=True,
            batch_size=batch_size,
...
class LinearClassifier(BaseClassifier):
    ...
    def forward(self, images: Tensor) -> Tensor:
        ...
        with torch.no_grad():
            ...
            # print_rank_zero(features.norm(dim=1)[:3])
    ...
    def on_train_epoch_start(self) -> None:
        ...
        # print_rank_zero("using eval mode")
        self.model.eval()
    ...
    def configure_optimizers(  # type: ignore[override]
        ...
        parameters = list(self.get_trainable_parameters())
        optimizer = torch.optim.AdamW(parameters, lr=self.lr, weight_decay=self.wd)
        scheduler = {
            "scheduler": CosineWarmupScheduler(
                optimizer=optimizer,
                warmup_epochs=int(
